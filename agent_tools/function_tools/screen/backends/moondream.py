...
class MoondreamBackend:
    ...
    @classmethod
    def load(cls, config: ScreenPipelineConfig) -> "MoondreamBackend":
        if config.provider != "moondream_local":
            raise ScreenToolError(
                "SCREEN_CONFIG_INVALID",
                f"screen provider 必须是 moondream_local，当前是 {config.provider!r}。",
            )
