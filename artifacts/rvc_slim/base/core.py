...
# LOCAL PATCH (Spica): now_dir anchored to this file (was os.getcwd()) -- the
# cwd is process-global and races with the GPT-SoVITS TTS pushd; everything
# below must resolve against the Applio root regardless of the current cwd.
# Do not overwrite on upstream sync.
...
def load_voices_data():
    with open(
        ...
        # evaluated at module import. Do not overwrite on upstream sync.
        os.path.join(now_dir, "rvc", "lib", "tools", "tts_voices.json"),
        "r",
        encoding="utf-8",
    ) as file:
...
def infer_spica_vocal(
    ...
    **_kwargs,
    ...
    _apply_infer_device(device)
    _message, output_path = run_infer_script(
        pitch=transpose,
        index_rate=index_rate,
        volume_envelope=volume_envelope,
        protect=protect,
...
def _apply_infer_device(device: str | None) -> None:
    normalized = _normalize_infer_device(device)
    if not normalized:
        return
    try:
        ...
        config.x_pad, config.x_query, config.x_center, config.x_max = config.device_config()
...
def _normalize_infer_device(device: str | None) -> str | None:
    if not device:
        return None
