...
# LOCAL PATCH (Spica): anchor download destinations to the Applio root instead
# of the process cwd (cwd race with the TTS pushd; a missing file used to
# create stray dirs under whatever cwd was current). Do not overwrite on
# upstream sync.
...
# LOCAL PATCH (Spica): destinations anchored to _APPLIO_ROOT (were cwd-relative)
folder_mapping_list = {
    "pretrained_v2/": os.path.join(_APPLIO_ROOT, "rvc", "models", "pretraineds", "hifi-gan"),
    "refinegan/": os.path.join(_APPLIO_ROOT, "rvc", "models", "pretraineds", "refinegan"),
    "embedders/contentvec/": os.path.join(_APPLIO_ROOT, "rvc", "models", "embedders", "contentvec"),
    "predictors/": os.path.join(_APPLIO_ROOT, "rvc", "models", "predictors"),
    "formant/": os.path.join(_APPLIO_ROOT, "rvc", "models", "formant"),
}
