...
def get_tts_wav(
    ...
    for i_text, text in enumerate(texts):
        ...
        if model_version not in v3v4set:
            ...
            if inp_refs:
                # 如果 inp_refs 是字符串（文件夹路径）
                if isinstance(inp_refs, str):
                    if os.path.isdir(inp_refs):
                        # 获取文件夹中所有音频文件
                        audio_extensions = ['.wav', '.mp3', '.m4a', '.flac', '.ogg', '.aac']
                        for ext in audio_extensions:
                            audio_files_to_process.extend(glob.glob(os.path.join(inp_refs, f"*{ext}")))
                            audio_files_to_process.extend(glob.glob(os.path.join(inp_refs, f"*{ext.upper()}")))
                    elif os.path.isfile(inp_refs):
                        # 如果是单个文件路径
                        audio_files_to_process = [inp_refs]
                # 如果 inp_refs 是列表
                elif isinstance(inp_refs, list):
                    # 处理原始的 gradio 文件对象或文件路径
                    for item in inp_refs:
                        if hasattr(item, 'name'):  # gradio 文件对象
                            audio_files_to_process.append(item.name)
                        elif isinstance(item, str) and os.path.isfile(item):  # 文件路径字符串
                            audio_files_to_process.append(item)

            # 处理文件列表
            if audio_files_to_process:
                for path in audio_files_to_process:
                    try:
                        refer, audio_tensor = get_spepc(hps, path, dtype, device, is_v2pro)
                        refers.append(refer)
                        if is_v2pro:
                            sv_emb.append(sv_cn_model.compute_embedding3(audio_tensor))
                        print(f"成功加载参考音频: {path}")
                    except Exception as e:
                        print(f"加载参考音频失败 {path}: {e}")
                        traceback.print_exc()

            # 如果没有成功加载任何参考音频，使用默认的参考音频
            if len(refers) == 0:
                refers, audio_tensor = get_spepc(hps, ref_wav_path, dtype, device,
                                                 is_v2pro)  # 修正：get_spepc 而不是 get_spemc
                refers = [refers]
                if is_v2pro:
