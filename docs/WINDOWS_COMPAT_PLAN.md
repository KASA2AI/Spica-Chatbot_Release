...
- **Status**: W0 侦察 complete（read-only，无代码改动、无 commit）；红队对抗审查 complete（read-only）；**W1 已批准（2026-07-05 审批窗口：HEAD 仍 `9acfbe6`、基线复测 1256 passed/1 warning/177 subtests 一致、全部代码锚点复校准零漂移；A1/A3/A8 裁决见 §0）；W2-a 已批准（2026-07-05 审批窗口：HEAD `c10133d`、基线 1293 passed/1 warning/177 subtests；裁决四条：①check_imports REQUIRED 全绿为硬 gate、PREFLIGHT 仅 WARN 留痕（PyAudio 属 PREFLIGHT）②`AppHost.initialize()` deferred 到 W2，W2-a 只做 import 闭包 smoke ③`docs/windows_bootstrap.md` 须回填真实 Windows smoke 留痕后才算 complete ④onnxruntime CPU 包在清单显式列出，不依赖 rapidocr 传递拉取）；**W2-a 已收口 complete（2026-07-05：真实 Windows 10 Enterprise LTSC 2021 机 smoke 全绿——REQUIRED 12/12、providers 含 CPUExecutionProvider、import 闭包 smoke `ok`、`platformName()` = `windows`；版本钉版零漂移；留痕见 `docs/windows_bootstrap.md` §3）**；**W2 已批准并施工（2026-07-05：Linux 实现落 main `5f6acc6`，全量 1310 passed/1 warning/177 subtests；Windows 真机 smoke 9/10 PASS——单屏 3840×2160@200% 下 locator/launcher/OCR 几何主链零偏移；多屏+混合 DPI 条目硬件 BLOCKED（单物理屏），严格口径裁决 W2 = partial / hardware-blocked 不标 complete；不阻塞 W3；W4/W5 前须补验或显式 waiver——挂账 §8 W2-DPI-MULTISCREEN，留痕 `docs/windows_w2_smoke.md`）**；**W3-a 已裁决（2026-07-05：断句选型 = webrtcvad 经 webrtcvad-wheels fork（原包无 cp311 win_amd64 wheel，实测）；recorder 契约 = mic_backend 字符串注入 + SpeechWorker 内 dispatch + P2-3 fatal markers「无法打开麦克风」条款；PTT 仅设计保留不实现，RMS 放弃）；W3 complete（2026-07-05：Linux 实现落 main `7bded62`，全量 1334 passed/1 warning/177 subtests；Windows 真机 smoke：check_imports 14/14 + 逻辑 smoke 21/21 + AppHost 选线日志 effective=windows/mic=generic + §6.2 交互 8/8 PASS；断句/转写真人链路在运行配置 cuda/float16 下验收，cpu/int8 warmup OK 作可移植补证不作硬 gate；真嗓 GPT-SoVITS 归 W4；参数留痕 vad_aggressiveness=2 / end_silence_seconds=0.9；留痕 `docs/windows_w3_smoke.md`）**；**W4 已拆分为 W4-a（GPU 环境探底 + W2-DPI 补验/waiver）+ W4-b（重型实装）——W4-a 已批准开工（2026-07-05 review：ALLOW W4-a only，HEAD `b206f51`；生产代码/tests/requirements 零改动，重型安装只进 probe/clone env；Linux 侧 wheel 探针已完成——RVC 关键包 7/7 + torch cu124 全有 py3.11 win_amd64 wheel，无 blocker；Windows 侧 A–E 段执行稿与回填区见 `docs/windows_w4_probe.md`，回填前不算 complete）——**W4-a 探底已回填完成（2026-07-05 真机：GPU 全链 CUDA+TRT EP 真 init 可达；关键产出 = onnxruntime-gpu EP 定序要求，建 CUDA/TRT session 前须 cuDNN9(torch/ct2 自带)+nvinfer(tensorrt_libs) 上进程 DLL 路径，坐实 A7「必须动 preload」；CPU→GPU 替换 runtime OK；W2-DPI 单屏→waiver 建议；`docs/windows_w4_probe.md` §5）；建议 W4-a 收口 + W4-b 放行，裁决归审批窗口**；**W4-b 已施工（Linux 实装落 main `84ee4b3`：OCR CUDA/TRT preload seam + requirements-windows-heavy + tests；随后 §6.3 真机 smoke 抓出并修复 preload 实质 bug——只强载 `cudnn64_9.dll` 不够，cuDNN-9 派发器按需 dlopen 的后端 DLL 不吃 add_dll_directory → EP 硬崩 `0xC0000409`，修复=强载整套 cuDNN-9 家族，补 Linux 回归钉；§6.3 A/B/D/E PASS、C(TTS真嗓)待单 env pin 清单、F/G/H 待真人；留痕 `docs/windows_w4b_smoke.md`）。**env 纠正**：生产 Linux `gptsovits` 单 env（numpy 1.26.4）已跑全部含 RVC，audio-separator numpy≥2 为元数据红/runtime 绿 → Windows 照单 env 复现（`worker_python` 默认 None 同 env），完整 TTS+RVC pin 清单从 gptsovits 导出（follow-on）。W4-b 仍非 complete**；W5 未批准**
...
| W2-a | Windows 基础 env bootstrap（requirements 清单 + import 冒烟） | W1 | 清单产出 Linux；装机+冒烟 Windows | **complete**（2026-07-05：artifact 落 main `c326d91`；真实 Windows 10 LTSC 机 smoke 全绿并回填 `docs/windows_bootstrap.md` §3——REQUIRED 12/12、CPUExecutionProvider 在、`platformName()` = `windows`；裁决见 Metadata Status 行） |
| W2 | Windows 真 WindowLocator + 原生 launcher + L3/L4 真机验收 | W1, W2-a | 实现/单测 Linux（落 main），验收 Windows（windows 分支） | **partial / hardware-blocked**（2026-07-05：Linux 实现落 main `5f6acc6`；Windows 真机 §6.1 smoke 9/10 PASS（单屏 200% DPI）；多屏+混合 DPI 硬件 BLOCKED → 挂账 W2-DPI-MULTISCREEN，**不标 complete**；不阻塞 W3，W4/W5 前须补验或显式 waiver；留痕 `docs/windows_w2_smoke.md`） |
| W3 | 入口 `.ps1`/`.bat` + 通用麦采集缝 + 软件断句 + WMF 播放验证 | W1（缝可与 W2 并行实现，验收需 W2-a 的 Windows 环境就位） | 实现 Linux，验收 Windows | **complete**（2026-07-05：W3-a 裁决 webrtcvad-wheels + recorder 契约先行；实现落 main `7bded62`（mic_backend 字符串注入 + generic_mic + run_spica.ps1，ps1-only）；Windows 真机 §6.2 smoke 8/8 PASS——真人链路 cuda/float16 口径、cpu/int8 warmup 补证、真嗓 W4-gated；留痕 `docs/windows_w3_smoke.md`） |
| W3b | ReSpeaker Windows 真机（可选） | W3 | Windows 真机 | optional（不阻塞主线） |
| W4-a | Windows GPU 环境探底 + W2-DPI 补验/waiver（拆自 W4 内容 1） | W2-a, W3 | 探针 Windows（probe/clone env）；文档 Linux | **probe complete / 待审批收口**（2026-07-05：Linux wheel 探针 7/7 无 blocker + Windows A–E 全回填 `docs/windows_w4_probe.md` §4——GPU 全链 CUDA+TRT EP 真 init 可达（需进程内 cuDNN9+nvinfer 上 DLL 路径，§5.3 定序）、CPU→GPU 替换 runtime OK/metadata 红、W2-DPI 单屏→waiver 建议；建议 W4-a 收口 + W4-b 放行 + §8 waiver 落定，**裁决归审批窗口**） |
| W4 | 重型运行时（TTS/RVC/Moondream/RapidOCR CUDA+TRT/faster-whisper CUDA）——**探底部分已拆出为 W4-a，余下实装 = W4-b** | W2, W3, W4-a | 几乎全 Windows+GPU | not approved（W4-b；前置 = W4-a 收口 + W2-DPI 补验/waiver 落定） |
...
- [x] conda env 按 `requirements-windows-base.txt` 创建完成，命令与版本留痕；（2026-07-05，Windows 10 LTSC 真机）
- [x] `python scripts/windows/check_imports.py`：**REQUIRED 全绿**；PREFLIGHT（PyAudio）失败仅 WARN 留痕，不阻塞收口；（PyAudio 实测也绿，providers 含 CPUExecutionProvider）
- [x] import 闭包 smoke：`python -c "from spica.host.app_host import AppHost; import ui.qt_overlay; print('ok')"`（`AppHost.initialize()` 是 W2 gate，不在本段）；（输出 `ok`）
- [x] PySide6 platform smoke：`QApplication([]).platformName()` 输出 `windows`；（输出 `windows`）
- [x] 以上输出原文回填 `docs/windows_bootstrap.md` §3。

### 6.1 W2 smoke（窗口/启动/采集几何）

> 2026-07-05 真机(Windows 10 LTSC,单屏 4K@200%,HEAD `5f6acc6`)执行:9 PASS / 1 BLOCKED,留痕 `docs/windows_w2_smoke.md`。

- [x] conda python 启动：`load_secrets()` 灌注 → `AppHost.initialize()` 完成，**选线日志行**（§3.3/P3-6）显示 effective platform = windows、三 lane 选中 windows；
- [x] overlay 正常显示：透明 + 置顶 + 无边框 + 点击穿透（`setMask` 打洞区域可穿透，控件区可点）；
- [x] 打开记事本（窗口化）→ 窗口枚举列出候选（含标题、hwnd）；
- [x] `manual_bind` 绑定记事本 → 绑定成功事件到 UI；（实测用 QQ 窗口走通全流程——记事本 CJK 标题撞既有 `guess_game_id_from_title` 限制，平台无关非 W2 回归，见留痕 §6）
- [x] 手填 exe 路径启动一个程序（`exe` launch_type，路径含反斜杠与空格各测一次——P3-1 佐证）→ 窗口出现并可绑定；（exe 与 command 整串两式均过）
- [x] 框选 OCR 区域（100% DPI 单屏）→ OCR loop 出字（CPU provider）且区域对齐；（实测本机 200% DPI，严于本条）
- [ ] **per-monitor DPI 验收**：150%（或本机实际倍率）+ 双屏各一次框选 → 区域对齐无偏移（L3/L4 gate）——**BLOCKED（硬件，单物理屏）**：「本机实际倍率」半条单屏 200% PASS（几何零偏移，留痕 §4）；「双屏/混合 DPI」半条无法执行 → 挂账 §8 W2-DPI-MULTISCREEN；
- [x] 遮挡触发暂停：用另一窗口盖住绑定窗口 / Spica overlay 盖住 OCR 区域 → session 暂停并提示；焦点离开 → 同理；
- [x] 最小化绑定窗口 → `WINDOW_MINIMIZED` 类暂停路径触发；
- [x] 关闭绑定窗口 → `WINDOW_GONE` 路径触发。

### 6.2 W3 smoke（入口/语音回环——TTS 允许 dummy/预生成 wav）

> 2026-07-05 真机（Windows 10 LTSC，HEAD `7bded62`）执行：8/8 PASS，留痕 `docs/windows_w3_smoke.md`。

- [x] `scripts/windows/run_spica.ps1`（或 `.bat`）冷启动成功（参数化 conda python）；（ps1-only，本段裁决不出 .bat）
- [x] 通用麦克风：说一句中文 → 软件断句收段（尾静音 ≈ `end_silence_seconds` 语义）→ faster-whisper（cpu/int8 配置）转写正确；（**实际运行口径 cuda/float16 PASS**——审批裁决：cpu/int8 是可移植建议口径非硬 gate，其 warmup OK 已作补证，不需另跑；参数留痕 vad_aggressiveness=2 / end_silence_seconds=0.9，见留痕 §6）
- [x] **无麦/禁用麦场景**：拔掉或禁用输入设备 → 报致命错误停止循环，**不进入无限重试**（P2-3 gate）；
- [x] 转写文本进 `run_turn` → LLM 回答返回；
- [x] 回答经 TTS 播放链播出（本段允许 dummy TTS / 预生成 wav 验证 `QMediaPlayer`+WMF；真嗓归 W4）；（真嗓 W4-gated，留痕 §7）
- [x] song 播放：一首 wav + 一首 mp3 经 song 播放器出声（WMF 编解码验证）；
- [x] 中文输入法（TSF）在输入框可用（无 ibus 依赖佐证）；
- [x] 打断：她说话时按停止 → 播放停止无 ghost 音频。

### 6.3 W4 smoke（重型 runtime，全链真值）

> 2026-07-06 真机（HEAD `84ee4b3` + preload cuDNN-家族修复）执行:A/B/D/E PASS（A/E 经修复）、C/F/G/H 待单 env pin 清单/真人。留痕 `docs/windows_w4b_smoke.md`。**preload 只强载 cudnn64_9 → EP 硬崩 0xC0000409 是本次真机抓出的实质 bug，修复=强载整套 cuDNN-9 家族。**

- [x] `python scripts/diag_ocr_providers.py`：CUDA EP 拉起（**修复后** det/cls/rec 全 CUDA 124ms；`rapidocr_trt_ep` 硬崩消失、TRT 引擎 build 失败但优雅降级 CUDA——判据可接受，如实记录）；
- [x] `python scripts/verify_watch_chain.py` 全链通过（`answer='画面上是个女孩。'`；尾部诊断脚本 teardown `WinError 32` 是跨平台 nit，非生产）；
- [ ] GPT-SoVITS：合成一句真语音 wav 并播出（首句延迟记录）——**⛔ 待单 env pin 清单**（依赖闭包深，`docs/windows_heavy_install.md` §3）；
- [x] faster-whisper `cuda/float16`：转写回环恢复 GPU（加载 3.9s + 推理 222ms；ct2 自带 cuDNN 不受 preload bug 影响）；
- [x] Moondream：`inspect_screen` 对真桌面出描述（**修复后**——装 transformers/accelerate/einops + app 装 provider seam + 下权重 → CUDA 出描述、无 ToolError；日文 prompt 输出退化另记）；
- [ ] galgame 全链：绑真游戏（窗口化）→ OCR（GPU）→ 总结/吐槽至少各触发一次 → 记忆写回可查——**⛔ 待真人**（OCR-GPU/E 组件已绿）；
- [ ] RVC/sing_song：点歌全链出声——**⏭️ 待单 env pin 清单装 RVC 层**（同 env，`worker_python` 留 None）；
- [ ] **完整语音回环真值版**：说话→STT(CUDA)→run_turn→GPT-SoVITS 真嗓→播出——**⛔ 待真人 + C**。
...
| W2-DPI-MULTISCREEN | **挂账（2026-07-05 严格口径裁决）**：W2 §6.1 条目 7 的多屏拓扑（跨屏原点/负坐标）+ 异构 per-monitor DPI（混合缩放）未验收——验收机仅单物理屏。已验边界：单屏 200% 全链零偏移（R2 的单屏高倍率部分已闭）；未验对象**只是**多屏坐标空间与混合 DPI 换算，勿用「单屏 150%」冒充补验 | **W4/W5 开工前必须补跑或另开审批窗口显式 waiver**。补验成本低：任意第二块显示器（电视 HDMI 亦可）+ 主副屏不同缩放率即构成完整场景（约 15 分钟，§6.1 条目 7 补勾 + `docs/windows_w2_smoke.md` 回填）；不阻塞 W3 |
