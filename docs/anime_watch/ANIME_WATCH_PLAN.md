...
- 下完停止做种；v1 不做上传管理承诺。
- **terminal decision ownership 与结果分离**：单锁/CAS owner 至少含 `RUNNING / MANUAL_CANCEL / STALL_CANCEL / SHUTDOWN_PRESERVE / COMPLETED`；事件结果独立为 `COMPLETED / CANCELLED / UNCONFIRMED / FAILED / PRESERVED`，并携带 `manual / stall / shutdown` cause，controller 不解析错误字符串决定语义。
...
- **queued-ready gap**：匹配 ID 的手停先写 suppress tombstone，再看 worker liveness。即使 worker 已退出但 matching ready 尚未被 GUI 消费，ready 仍可登记并保留完成文件，却不播放、不确认；tombstone 只由 matching ready 消费，不能被 `finished` 提前清除。
- **completion-safe cancel（fail-closed 最佳可用保证）**：
  1. `/app/version` 分派：qBT 4.1–4.6 仅调用 `torrents/pause`，5.x 仅调用 `torrents/stop`；未知版本零 mutation。4.1 不假定支持 `torrents/info?hashes=`，统一按 category 拉取后本地 exact-hash 匹配。
