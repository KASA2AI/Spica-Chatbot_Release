...
> 状态：**Phase 0–5A 已落地；2026-07-11 safe cancellation hardening 已完成自动化实现与回归，真实 qBittorrent/播放器验收仍按 §10 真机矩阵独立执行。**
...
>
> **2026-07-10 运行纠偏**：Phase 0 的“只拼 magnet”在 qBittorrent DHT 未启动时会长期停在 `metaDL`，并丢失 Mikan 原种的 tracker tier。生产链现改为：搜索阶段仍只解析 RSS；仅对最终选中的同源 `.torrent` 禁重定向、限 1 MiB 获取，校验原始 `info` 字节的 SHA-1、tracker/辅助网络字段后，以内存 multipart 上传 qBittorrent。旧事件仍保留 magnet 兼容路径；绝不把 RSS URL 或本地路径交给 qBittorrent。
>
> **2026-07-11 取消协议修订**：语音/文字的 `cancel_anime_download` 工具与 UI 停止按钮统一进入 `AnimeController.cancel_current_download(expected_request_id=...)`；工具供给和按钮按下时都冻结 opaque request ID，缺失、陈旧、重复或跨线程调用一律 fail closed。qBittorrent 的删除采用版本分派的 freeze→有界复读→删除→消失确认协议；这是最佳可用保证，不是事务/CAS，最终复读与删除之间仍有外部 resume 的残余竞态。
...
| D5 | 完成行为 | **智能阈值**（默认 50s）：`≤50s` 安全后自动播；`>50s` 由 Spica 走系统 turn 确认 |
...
| #2 跨线程只走 RuntimeEvent | host→UI 只 emit typed `AnimeRequestEvent`（触发下载）与 `AnimeCancelRequestEvent`（绑定 ID 的停止请求）。UI→host 的状态回流使用注入 seam（P1-6，controller 上报 task/library 结果）。`AnimeReadyEvent` 与下载进度产生、消费都在 UI worker/controller 内，走 Qt signal，不伪装成 host→UI 通道（P2-19） |
...
spica/ports/
  ...
  torrent_client.py      # TorrentClientPort 白名单动作面（P0-3 收口）：
                         ...
                         #   add_torrent_bytes(payload, expected_infohash)
                         #                               -- 仅内存字节；二次校验 hash/网络字段
                         #   status(task_id) / cancel(task_id)  -- 强制 category=spica-anime + exact hash；
                         #   cancel 返回 typed cancelled/already_completed/missing outcome
...
spica/adapters/tools/
  ...
  cancel_anime_download.py
                         # act 工具；availability 绑定本轮 opaque request ID，handler 一次性消费
...
spica/core/
  anime_events.py        # AnimeRequestEvent / AnimeReadyEvent / AnimeCancelRequestEvent
                         # （跨 host→UI；仿 song_events.py）
...
ui/
  ...
  controllers/anime_controller.py  # 消费 request/ready/cancel、安全延后、起播、状态 chip；
                                    # 语音/文字事件与 UI 按钮共用 request-ID guarded 取消入口
```
...
**不碰**：`run_turn` / `stages.py` / `orchestrator.py` / `tool_round.py` / `prompt_builder.py` / `ChatEngine`（`stream_system_turn` 现成，零改动）/ `MemoryPort` / `MemoryScope` / recent memory / galgame 全域 / registry 机制本身 / `proactive.py`（P1-5：不实现 `queue_latest` 保留字段，空闲调度在 controller 侧做）/ 冻结链 / v1 `LLMPort` / `domain_router.py`（v1 无 turn binding）。
...
- **不写** recent memory 特例。
- **不改** `proactive.py`（P1-5：短周期空闲调度在 controller 侧，不动 arbiter 语义）。
...
用户:「spica我想看无职转生第三季第一集」
  ...
  → 完成 → AnimeReadyEvent → controller 先定 consent intent，再过安全执行门（2026-07-11 修订）:
      耗时 ≤ 50 秒 → AUTO_PLAY intent；busy / galgame 活跃只延后，空闲后仍自动播
      耗时 > 50 秒或未知 / 启动恢复 → REQUIRE_CONFIRMATION intent
      确认遇 busy → controller 用单个 250ms one-shot timer 等空闲后发 system turn；
         用户先开口只延后，不删除 intent；若「放吧」已播放则 is_played 消费
```
...
- **RSS hash 仍是信任锚，但生产下载保留原种 tracker tier（2026-07-10 纠偏）**：`enclosure@url` / `Home/Episode/` 的 40 位十六进制是 btih；搜索阶段据此构造兼容 magnet。只对 resolver 最终选中的候选，从配置过的同源 `/Download/.../<同一hash>.torrent` 禁重定向、流式限量获取，严格解析并按原始 `info` bencode 字节复算 SHA-1；拒绝 hash 不符、非公网/畸形 tracker、重复键及 `url-list`/`httpseeds`/`nodes`。验证后的字节经 RuntimeEvent 以 base64 传递，再由 qBittorrent adapter 二次校验后 multipart 上传。RSS URL 和本地路径都不进入 torrent 动作面。
...
- Mikan 新任务优先上传已验证的原始 `.torrent` 字节，保留其 tracker tier 并直接获得元数据；历史 RuntimeEvent/pending 无 payload 时继续走严格 magnet 兼容路径。qBT `metaDL` 映射为独立 `metadata` 状态，UI 显示“正在获取种子元数据”，不再伪装成普通 0% 下载。
- **只认领 `spica-anime` 类别的任务**，所有 status/cancel 强制类别过滤（P2-20），绝不动用户手动种子。
- 下完停止做种；v1 不做上传管理承诺。
- **terminal decision ownership 与结果分离**：单锁/CAS owner 含 `RUNNING / MANUAL_CANCEL / STALL_CANCEL / SHUTDOWN_PRESERVE / COMPLETED / FAILED`；事件结果独立为 `COMPLETED / CANCELLED / UNCONFIRMED / FAILED / PRESERVED`，并携带 `manual / stall / shutdown` cause，controller 不解析错误字符串决定语义。
  - manual 先认领后，shutdown/timeout 不得覆盖；shutdown 先认领后，后续绝不 mutation qBT。
  - stall 仅可从 `RUNNING` 认领；每个破坏性调用前再次核对 owner。
  - 删除前观察到完成永远保留；manual 后才观察到完成仍登记最终文件，但保留 suppress 语义：不 autoplay、不 confirmation。
  - manual 在 `add_magnet`/`add_torrent_bytes` 阻塞期间被接受，而 add 随后以结果不明失败时，用 locator/info payload 中已知 infohash 进入同一 typed disambiguation；无法确认则 `UNCONFIRMED/MANUAL` 并提示后台任务可能仍在，绝不退化为 `FAILED/NORMAL`。
- **用户取消入口与身份**：`cancel_anime_download` 仅在有活跃下载时供给。availability 每次先清 TLS，再绑定当轮 immutable request ID；handler 入口先 pop，再与 live ID 精确比较。missing、A→None、A→B、forced、duplicate、跨线程以及 sink failure 后重试全部返回 stale ToolError、零事件，绝不执行时重绑当前任务。事件只携带 offered ID。UI 按钮在 `pressed` 捕获用户看到的 ID，`clicked` 始终将其传给同一个 controller 入口；RuntimeEvent 保留同样的 stale-ID guard。
- **queued-ready gap**：匹配 ID 的手停先写 suppress tombstone，再看 worker liveness。即使 worker 已退出但 matching ready 尚未被 GUI 消费，ready 仍可登记并保留完成文件，却不播放、不确认；tombstone 只由 matching ready 消费，不能被 `finished` 提前清除。`ready→finished` 与 `finished→ready` 两种 signal 顺序都会清理 terminal worker identity，不能污染未来 Python id 复用。
- **completion-safe cancel（fail-closed 最佳可用保证）**：
  1. 每次可能发生破坏性 mutation 的 `cancel()` 都重新读取 `/app/version`，不得跨调用缓存协议：qBT 4.1–4.6 仅调用 `torrents/pause`，5.x 仅调用 `torrents/stop`；未知版本零 mutation。4.1 不假定支持 `torrents/info?hashes=`，统一按 category 拉取后本地 exact-hash 匹配。
  2. freeze 后有界轮询；v4 只接受 `pausedDL`，v5 只接受 `stoppedDL`。任一复读见 `progress >= 1` 或 UP-family（含 `stoppedUP`）即完成优先、保留文件。`error/missingFiles` 只有在 freeze 成功、同 hash/category、`progress < 1` 且稳定复读后才可继续；checking/moving/unknown/超时全部 fail closed。
  3. worker 已持有 task ID，因此 adapter `cancel` 的初始 category miss，以及初始 owner 确认后的任何 category read 消失，都先用 global exact-hash properties 区分真实移除（404）与外部 recategorize（200）；后者 `CANCEL_OWNER_LOST` fail closed、零 delete。worker 的 final status `TASK_NOT_FOUND` 也必须进入同一 typed cancel 端口，不能自行宣告成功。delete 前再次确认 worker terminal owner，并做 category+exact-hash 最新复读；只对确认未完成且仍处安全冻结态的数据发送 `deleteFiles=true`。mutation 的 HTTP 200/204 只表示请求被接受，不能证明已停止/删除。
  4. delete 后用 4.1/5.x 均支持的全局 `torrents/properties?hash=<exact>` 有界复读（404=任务不存在），避免外部 recategorize 冒充删除；只有确认任务消失才返回 `CANCELLED`，仍存在返回 `UNCONFIRMED`。adapter 最新复读已完成时返回 completed/save_path，绝不删除。
  5. Web API 无条件删除没有 CAS；最终复读与 delete 间的外部 resume/recategorize、同 hash remove/re-add、qBT/libtorrent in-flight completion 仍是残余竞态。任务消失也不证明磁盘 unlink 已同步完成；文案只声明“qBT 任务已移除并请求删除数据”，不宣称原子/事务或同步磁盘删除。
- **生命周期（P1-9）**：
  1. 普通 shutdown 先认领时仅停本地 polling，qBT 外部任务保留；**yt-dlp 子进程**终止并保留 `.part`，任何取消/卡死路径都不删除 B 站 `.part`。
  2. **重启 inactivity**：合法 qBT `last_activity`（Unix epoch seconds）存在时独占作为 anchor；只有缺失/非法才回退 pending `created_at`。首次 progress 只作 baseline；transient/final recheck 恢复不得抹掉已累计 inactivity。future/毫秒/clock-skew 值按 fresh fail safe，不因此立即破坏性删除。
  3. **启动对账**：按类别对账「qBT 已完成但 library 未登记」→ **只登记、走确认队列，永不 auto-play**（防开机突然弹播放器）。
- 卡死：`stall_timeout_minutes`（默认 **10 分钟**）无真实进度后做 final status recheck；完成绕过取消，真实增长重置计时，其余确认未完成状态（包括 `error`）进入上述 scoped cancel。取消失败仍释放本地 single-flight，并明确提示“后台任务可能仍在”。不会自动换源；换源只能由后续明确用户请求重新发起。
- qbt 轮询中断连/重启：**重连而非判失败**（P1-10）。

### 5.5 完成行为（D5）与「放吧」闭环

- worker 记录起始时刻；`playback_policy`（纯函数，spica/anime/）判耗时。
- **2026-07-11 真机修订：同意策略与安全执行拆开。** 有效耗时 `≤50.000s` 固定为自动播放，`>50.000s`、未知耗时和启动恢复固定为确认；busy / galgame 不得改写 consent intent。
- **auto-play 执行前仍过忙态真值 + galgame 活跃判定（P1-7）**：正在 TTS 说话/唱歌、用户正在开口或 galgame 陪玩进行中时只等待；安全后自动播放，不弹窗打断、不降级确认。
- 确认走 `ProactiveTurnRequest`（drop_if_busy）。controller 以按 episode 去重的 pending action + 单个短周期 one-shot timer 等待聚合空闲；`try_speak` 若在竞态中返回 False，保留原项短周期再检。用户活动不删除确认；若用户已说「放吧」并成功播放，`is_played` 清掉待处理项。不改 proactive.py。
...
- `cancel_anime_download` 同为 `effect="act"`、`chainable=False`、无参数 strict schema；只在 active request + UI sink 同时存在时供给。其 offer identity 是 thread-local one-shot capability，不修改 registry/runtime/tool_round/run_turn。语音和文字触发该工具后 emit `AnimeCancelRequestEvent(request_id=offered_id)`；UI bridge 与停止按钮最终都调用 controller 的同一个 `expected_request_id` 入口。
...
anime:
  ...
  download_dir: "static/generated_anime"  # 默认;相对锚 repo 根(resolve_data_path)、gitignore;按番名分子目录 <anime>/<ep>（P2-14 绝对路径传 qbt）
...
| 磁力长时间 0% / 卡进度 | 默认 10 分钟 hard cutoff → completion-safe scoped cancel；**不自动换源** |
...
| 用户停止 B 站下载 / 普通退出 | 终止本地 yt-dlp，**保留 `.part`** 供未来显式重试；不自动换源、不自动删除 `.part` |
| qBT freeze/delete 无法确认 | typed `UNCONFIRMED`，释放本地 single-flight，并明确提示后台任务可能仍在 |
...
- `test_anime_events`：`AnimeRequestEvent`/`AnimeReadyEvent`/`AnimeCancelRequestEvent` 经 `register_event` 解码 round-trip；ready 的 typed result/cause 与 legacy 缺字段派生（P2-16）。
- `test_cancel_anime_download_tool` + assembly persistence：真实 `run_turn` 的 offer A→执行前 active B stale red；forced/missing/A→None/A→B/duplicate/decline/sink failure/cross-thread/concurrent turns 全部零误发。
- `test_anime_completion_retry`：**busy 时零请求→短周期等空闲→最终确认；arbiter 竞态不丢**（P1-5）。
- `ui` 测试（`pytest.importorskip PySide6`）：`anime_worker` 用 `ManualClock` 钉 599.9/600 边界、terminal CAS race、restart `last_activity`、hard-cutoff error、completion-safe outcome、子进程退出保留 `.part`；`anime_controller` 覆盖 press A→state B→click、ready queued/finished gap tombstone、manual-completed 零 autoplay/confirmation、`UNCONFIRMED` 释放 single-flight（P1-9/P2-16 / guardrails §3.2）。
- 既有守门：`test_registry` / `test_resolved_config_equivalence`（基线更新）/ `test_no_getenv` / 全量 `python -m pytest tests -q`。

验收矩阵：

| 层 | 自动化（提交门） | 真机（独立人工验收，不由单元测试代替） |
| --- | --- | --- |
| tool / identity | mock UI + 真实 `run_turn`；A→B、并发、跨线程、重复与失败重试 | 下载 A 时语音/文字说“停止”；状态切换压力下确认绝不停止 B |
| UI / terminal | Qt fake worker；press/click ID、manual/shutdown/stall/completion CAS、queued ready tombstone | 点击 A 的停止按钮后立即制造 ready/窗口事件交错；完成文件保留且零 autoplay/confirmation |
| qBittorrent | mock Web API 的 4.1/4.6/5.x 版本与状态矩阵；**不连接本机服务** | 在隔离测试 category 分别验 4.6 与 5.x freeze 状态、完成竞态、删除后任务消失；记录外部 resume 残余竞态，不宣称原子性 |
| restart | ManualClock + qBT epoch/pending fallback、transient recovery | 重启恢复未完成任务；核对 `last_activity` inactivity 与完成对账不自动播放 |
| B 站 | fake subprocess；取消/退出均保留 `.part`、不播放 `.part` | 真实 yt-dlp 中途停止/退出后确认 `.part` 仍在，且没有自动换到蜜柑 |
| 回归门 | targeted → cross-phase → guards → resolved-config diff → `python -m pytest tests -q` | 真实 space/Mikan 下载、VLC/Totem、cookie 高清、「最新一集」、「放吧」闭环 |

真机验收必须使用隔离的测试任务/category；不得把开发自动化指向用户真实 qBT、真实数据库或备份目录。
...
| P1-5 drop_if_busy 语义写反 + 丢播报无补救 | §5.5（controller 持久化 intent + 短周期空闲调度，不改 proactive.py）+ §10 completion scheduling tests |
| P1-6 缺 UI→host 状态回流 seam | §2 铁律#2 + §3.1（host 持 library 唯一写点，worker 注入回调上报）+ §5.4 |
| P1-7 auto-play 打断唱歌/触发 privacy gate | §5.5（consent 不变；controller 等 busy/galgame 安全后执行）+ §10 policy/controller tests |
