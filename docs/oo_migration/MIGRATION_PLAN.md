...
- **Version**: v2
- **Date**: 2026-07-03
- **Status**: Phase 0 / 0D / 1 / 2 / 3 / 4R / 4 / 5 / 6a / 6b / 7 / 8 complete——Y1 主线 + Phase 8 seam 基建全部收口，D3 已停钟；Phase 9 feature-triggered（等 browser/media 立项）；co-watch **feature 本身**仍未立项（Phase 8 只交付了它的 seam）
...
| 8 | ActiveDomainRouter + WindowTarget/PrivacyGate + request 落点泛化 | 3, 4 | **complete** | 已收口 `e7bbb7f`（裁决 `b2b9c32`；链 c0 `8b83657` → c1 `cf6c415` → c2 白名单 `c5647d1` → c2 实现 `e7bbb7f`）；context.py/chat_engine.py 受控刀已落且 galgame lane 字节等价；co-watch feature 本身未实现 |
...
- **approval status**: **complete（2026-07-04 收口 `e7bbb7f`）**——seam 基建获提前批准并按 c0→c3 分段施工完毕（设计裁决 `b2b9c32` + c2 白名单 amendment `c5647d1`）；exit conditions 逐条达成见 PROGRESS 条目；**co-watch feature 本身未实现、未立项**（它到来时直接消费本 phase 的 router/泛化槽/WindowTarget/PrivacyGate，AppHost ≤15 行、runtime 0 行预算生效）。
...
- **分段施工（c0 → review → c1 → review → c2 → review → c3，各段独立 commit/revert）**:
  ...
  - **8-c2**: window.py + privacy_gate.py + ocr_loop/watch_game_screen 改调 + 裁决 4/5 的 gate 单测。**（2026-07-04 白名单缺口 amendment：8-c1 收口后实跑爆炸半径 rg 认定两个裸 5 元组 provider 必改点，正式入 8-c2 限定白名单）** ① `tests/test_chat_tool_round.py`——**仅允许**把 `WatchGameScreenTool` 的 provider 从裸 5 元组适配为 `WatchContext(WindowTarget(...), locator, capture, state)` 形状（`:203-208`）；**不得**改工具轮断言/LLM 调用断言。② `scripts/verify_watch_chain.py`——**仅允许**同形 provider 适配（`:196-201`）；**不得**重写诊断链逻辑。**排除误报（不入白名单）**：`tests/test_note_game_observation.py:316` 与 `tests/test_companion_controller.py:331-346` 命中的是 controller `current_watch_target()` 的 `(game_id, window_id)` **二元组**——controller 本轮不改、二元组形状不变，零影响；
  - **8-c3**: docs 收口（README/PROGRESS/MIGRATION_PLAN 状态 + CLAUDE/GUARDRAILS 改向）。
- **批准前置检查（爆炸半径 rg）**: `rg -n "GameTurnBinding|set_game_binding_provider|current_game_context|current_watch_target|_companion_watch_context|game_context_request" spica tests scripts ui hardware agent_tools`——全部命中文件必须已在白名单内或证明零改动；含 scripts（7 期教训制度化）。**2026-07-04 实跑结论（8-c2 开工前）**：必改命中恰两处（`test_chat_tool_round.py:203-208`、`verify_watch_chain.py:196-201`，均为直构工具的裸 5 元组 provider，已入上方 8-c2 限定白名单）；`test_note_game_observation.py` / `test_companion_controller.py` 命中为 controller 二元组（零改动，见 8-c2 排除注记）——实施窗口按纪律停工上报后经本 amendment 扩权，未现场扩权。
...
- **characterization tests to add or update**（2026-07-04 裁决落盘）: 8-c0 特征套件；router 单测（publish/retract/priority/平手 WARNING/no-throw 契约）；binding 泛化契约（GameTurnBinding lane 字节等价 + DomainTurnBinding lane 入 tuple + **双 lane §27①** + **非 galgame binding 不入 galgame 闭包【修正 1】** + **exploding sink 无残留【修正 3】**）；gate 单测（**OVERLAY_COVERS 保持【修正 4】** + **owner_domain 校验【修正 5c】** + watch 不对称保留）；非 galgame contributor 禁读 `game_context_request` 守卫（**2026-07-04 二轮审查 NEW-1 补交付**：`tests/test_game_context_slot_guard.py`——三形态 AST 扫 attr/getattr/kwarg + 形态级白名单双向精确相等，比原承诺更强）。
- **co-watch 立项前置（2026-07-04 二轮审查共识挂账）**: 前缀注册（`context.py` 一行入 `DOMAIN_CONVERSATION_PREFIXES`）必须与首个使用该前缀的 caller **同 commit**，并同批加 double-wrap 回归——否则中间态 `cowatch::` caller 会被活跃 galgame binding 改写进 galgame 会话（前缀未注册即不受 double-wrap 保护，裁决 2 既定语义）。
- **required test gate**（2026-07-04 补 8-c2 组）: `python -m pytest tests -q` 全量绿，重点关注 galgame 全族、`test_watch_game_screen`、新 gate 单测；8-c2 追加：① `python -m pytest tests/test_privacy_gate.py tests/test_watch_game_screen.py tests/test_ocr_loop.py -q`；② `python -m pytest tests/test_chat_tool_round.py -q`（provider 形状适配后断言零改动全绿）；③ `python -m pytest tests/test_domain_binding_contract.py tests/test_domain_router.py -q`；④ `python scripts/verify_watch_chain.py` 离线构造/活体验收（脚本为 fake LLM/TTS/locator/capture/analysis 的离线活体诊断，**不依赖外部窗口环境**，无 `--help` CLI）——适配后应走到 `WatchContext(WindowTarget(...), locator, capture, state)` 形 provider 并全链跑通。
