...
- **exit conditions**（2026-07-03 计划修订）: `orchestrator` / `tool_round` **完整禁面十名零命中**（AST 守卫全绿）、零 `services.llm_client` 引用；`test_no_dict_config` TEMP_EXEMPT 清空；Phase 0 #3（`test_responses_probe_shape`）**零修改**保持绿；D3 停钟。
