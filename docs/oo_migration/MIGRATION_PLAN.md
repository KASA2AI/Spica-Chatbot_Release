...
- **Version**: v2
- **Date**: 2026-07-03
- **Status**: Phase 0 / 0D / 1 / 2 / 3 / 4R / 4 / 5 / 6a / 7 complete，**D3 已停钟**（Phase 7 于时钟内完成）；Phase 6b not approved / 可选（批准前须先裁决其「施工前必须裁决」小节）；Phase 8 / 9 feature-triggered
...
| 6b | ModelRouter 收编 host endpoint 决策 | 6a | not approved | Phase 7 已完成，6b 仍未批准/可选（D3 已停钟）；批准前须先裁决「施工前必须裁决」小节 |
...
- **施工前必须裁决（2026-07-03 计划修订预登记，批准 6b 前必须先出 plan amendment 裁决，不得现场扩权）**: `for_role("judge") -> BoundModel` 与 `_judge_llm_adapter` patch-validity 存在设计张力——若 router 全责构建 adapter+model，`patch.object(AppHost, "_judge_llm_adapter", ...)` 将不再拦截真实构造（正是 Phase 4 以 PatchValidityTest 拦死的 no-op patch 复发形态）。候选解法三选一，选定后相应重校准白名单与 patch 目标：
  ...
  - **BUG-4（2026-07-04 审查追加，须随方案一并裁决）**: `replace(deps, llm=…/config=…)` **不重铸** `deps.model` 绑定（`deps.py` 契约注释 + `test_replace_with_new_llm_keeps_old_binding_by_design` 已钉现状）——6b/换模型路径必须显式传新 BoundModel 或重建 deps；遥测分叉（`agent_model` live-read config vs 请求走 bound 值）随同对齐。
