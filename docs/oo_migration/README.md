...
| 8 | ActiveDomainRouter + WindowTarget/PrivacyGate + request 落点泛化 | 3,4 | **co-watch feature 批准**（seam 基建获提前批准施工） | 已收口 | `e7bbb7f2ca9d0466f45467080d1d85505c23815c` | 2026-07-04 收口（设计裁决 `b2b9c32`；链：c0 `8b83657` → c1 `cf6c415` → c2 白名单 `c5647d1` → c2 实现 `e7bbb7f`）：ActiveDomainRouter（ChatEngine 单槽唯一注入者）+ 双 lane request 泛化（`DomainTurnBinding`/`domain_context_requests`，galgame legacy lane 字节等价）+ WindowTarget/WatchContext + PrivacyGate（ocr/watch 双 purpose，watch 不对称保留，check→capture race 挂账未收窄）已全部落地；**co-watch feature 本身未实现、未立项**。全量 gate 1245 passed, 170 subtests。2026-07-04 二轮对抗审查（fable-max+gpt 双窗口）：P0/P1 零、主链判可信；NEW-1..6 小修同日落地（NEW-1 守卫补交付 `test_game_context_slot_guard.py` + v1 守卫扫描域扩 runtime + memory truthiness + fail-open/unknown-role loud 化 + tie 口径对齐），基线 → **1256 passed, 177 subtests**。详见 `PROGRESS.md` 审查后小修条目 |
