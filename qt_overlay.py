...
from tts_service import GPTSoVITSTool
from visual_service import VisualDiffService
...
class StartupWarmupWorker(QThread):
    ...
    def run(self) -> None:
        try:
            ...
            self.status_changed.emit(f"正在预热 {emotion} GPT-SoVITS 模型...")
            result = self.tts_tool.warmup(emotion=emotion)
            duration_ms = float(result.get("duration_ms") or 0)
            self.finished_ok.emit(f"{emotion} GPT-SoVITS 模型已就绪（{duration_ms:.0f}ms）。")
        except Exception as exc:
...
class OverlayWindow(QWidget):
    ...
    def _maybe_advance_playback(self) -> None:
        ...
        QTimer.singleShot(180, self._play_next_tts_item)
