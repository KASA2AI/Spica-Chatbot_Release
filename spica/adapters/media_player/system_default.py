...
class _OpenerNotFound(MediaPlayerError):
    """The opener executable was not found on PATH (distinct from a launch that
    started then failed). Lets ``_open`` fall back to the OS default opener on
    Windows, where a bare ``player_command`` like "vlc" is not on PATH (VLC's
    installer does not add itself). Carries code OPEN_FAILED so an uncaught one
    behaves like the old error."""

    def __init__(self, detail: str) -> None:
        super().__init__("OPEN_FAILED", detail)
...
class SystemDefaultPlayer:
    ...
    def _open(self, path: str) -> None:
        if self._player_command:
            try:
                ...
                # (the user asked for it); on Windows a bare "vlc" is expected to be
                # missing (VLC's installer doesn't add itself) -> fall back to the OS
                # default opener rather than failing the play (W4-b §6.3 real machine).
                if not self._platform.startswith("win"):
                    raise
                _LOGGER.warning("player_command %r not found on PATH; falling back "
                                "to os.startfile", self._player_command)
    ...
    def _run_argv(self, argv: list[str]) -> None:
        try:
            ...
            raise _OpenerNotFound(str(e))
        except OSError as e:                             # other launch failure (perms, ...)
            raise MediaPlayerError("OPEN_FAILED", str(e))
