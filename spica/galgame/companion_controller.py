...
def guess_game_id_from_title(title: str | None) -> str:
    """Default game_id from a window title: the leading latin-alphanumeric run,
    lowercased. "LimeLight Lemonade Jam" -> "limelight"; "anemoi gemini-3.1" ->
    "anemoi". Returns "" when the title has no leading latin run (e.g. a CJK title)
    -- the caller must then pass an explicit game_id. This is only a DEFAULT; the
    caller's game_id always wins."""
    if not title:
        return ""
    match = re.match(r"[0-9a-z]+", title.strip().lower())
