...
``BoundModel.complete`` (model port v2, OO migration Phase 6a), and parses a
structured verdict. It NEVER builds a
...
class GalgameReactionJudge:
    """Clone of ``GalgameSummarizer`` shape: one ``BoundModel.complete`` call
    per verdict (the bound value carries the model), JSON parsed tolerantly.
    Holds no state, no Qt, no run_turn coupling."""
