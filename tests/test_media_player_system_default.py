...
def test_player_command_oserror_raises(tmp_path):
    # Linux: a configured player that isn't on PATH is a real error (no fallback).
    d = _dir(tmp_path)
    f = _mkv(d)
...
def test_windows_player_command_not_found_falls_back_to_startfile(tmp_path):
    # Windows: the default player_command "vlc" is NOT on PATH (VLC's installer
    # doesn't add itself) -> Popen raises FileNotFoundError -> fall back to
    # os.startfile instead of failing the play (W4-b §6.3 real machine).
    d = _dir(tmp_path)
    f = _mkv(d)
    opened: list[str] = []
    run = Popen(exc=FileNotFoundError("'vlc' not found"))
    SystemDefaultPlayer(str(d), player_command="vlc", platform="win32",
                        popen=run, startfile=opened.append).play_file(str(f))
    assert opened == [str(f.resolve())]                  # fell back to os.startfile


def test_windows_player_command_launch_failure_still_raises(tmp_path):
    # A player that STARTS then dies (nonzero rc) is a real failure, NOT
    # "not found" -> must surface, not silently fall back to startfile.
    d = _dir(tmp_path)
    f = _mkv(d)
    opened: list[str] = []
    run = Popen(returncode=3)
    with pytest.raises(MediaPlayerError) as ei:
        SystemDefaultPlayer(str(d), player_command="vlc", platform="win32",
                            popen=run, startfile=opened.append).play_file(str(f))
    assert ei.value.code == "OPEN_FAILED"
    assert opened == []                                  # no fallback on a real launch failure
