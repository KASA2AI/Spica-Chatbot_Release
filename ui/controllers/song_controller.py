...
class SongController(QObject):
    ...
    def handle_intent(self, intent: SongIntent) -> bool:
        ...
        if intent.action == SongAction.SEARCH:
            ...
            self._sync_pending_song_hint_from_context()
    ...
    def _clear_pending_song_hint(self) -> None:
        ...
        self.ui_state.pending_song_hint = None

    def _sync_pending_song_hint_from_context(self) -> None:
        context = self.ui_state.context
        if not (context.pending_song_raw_query or context.pending_song_artist or context.pending_song_style):
            self.ui_state.pending_song_hint = None
            return
        self.ui_state.pending_song_hint = {
            "raw_query": context.pending_song_raw_query,
            "artist": context.pending_song_artist,
            "style": context.pending_song_style,
        }
